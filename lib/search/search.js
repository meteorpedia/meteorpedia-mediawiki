...
function prepareContent(content, query) {
	...
	if (query.match(/\(.*\)/)) {
		...
		if (match)
			...
			// really short or some other reason regex didn't work
			return '1'+content.substr(0, 400);
	} else {
		content = content.replace(/● /g, '●&nbsp;');
		return '2'+content.substr(0,400);
	}
}
...
if (Meteor.isServer) {
	...
	Meteor.publish('mwSearch', function(query) {
		var self = this;
		...
		edits = WikiEdits.find({
			...
			limit: 20 // infinite scroll coming soon
		});
	...
	Meteor.startup(function() {
		...
		Deps.autorun(function () {
			...
			Meteor.subscribe('mwSearch', query);
