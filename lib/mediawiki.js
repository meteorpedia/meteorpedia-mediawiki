...
var escapeRegExp = {
	...
	variable: /\{\{\{(.*?)\}\}\}/g 			// handle at the end
};

/**
 * This will either escape (with placeholders) or remove code which you
 * probably want to ignore, e.g. <pre>, <code>, <nowiki>, {{{variable}}}.
 *
...
function clientRender(data) {
	...
	var pageNameInURL = location.pathname.substr(6); // strip /read/
	if (pageNameInURL != data.page.name && redirectedFrom[data.page.name]) {
		data.edit.contentParsed
			= '<div id="contentSub">(Redirected from '
			+ '<a _class="internal-link" data-link="read" data-name="'
			+ redirectedFrom[data.page.name] + '#redirect=no" '
			+ 'href="/read/' + redirectedFrom[data.page.name]
			+ '#redirect=no">' + redirectedFrom[data.page.name]
			+ '</a>)</div>\n' + data.edit.contentParsed;
	} else
		redirectedFrom = {};
...
function parse(content, placeholder) {
	...
	if (content.match(/^#[A-Za-z]*:/)) {
		...
		for (var i=1; i < parts.length; i++) {
			...
			if (!assignment[1]) {
				assignment[1] = assignment[0].trim();
				assignment[0] = i; // {{{1}}} etc
...
if (Meteor.isClient) {
	Meteor.startup(function() {
		...
		Template.edit.rendered = function() {
			...
			Template.mwEditHelp.deps = function() {
				var page, pid = pageId();
				if (!pid)
					return []; // new page

				page = WikiPages.findOne(pid);
				return page.deps;
			}
