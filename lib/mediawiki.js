...
Extensions.add({
	...
	hooks: {
		         'render': { api: '1.0.0', func: renderFuncs, priority: -1 },
...
// remove (potential) problems by replacing them with placeholders (or removing)
function notouch(content, placeholder) {
...
function updateDeps(data) {
	var content = notouch(data.edit.content);
...
function serverParse(data) {
	...
	var content = notouch(data.edit.content, placeholder);
...
// TODO, the entire section below will be cleaned up to run off hooks
function renderFuncs(data) {
	...
	data.edit.contentParsed = content;
	hookData = { page: data.page, edit: data.edit, placeholder: placeholder, toolbar: toolbar };
	hookData = Extensions.runHookChain('mwClientRenderPreMD', hookData);

	// Best time for MarkDown is now: after functions and before unwrap
	hookData.edit.contentParsed = marked(hookData.edit.contentParsed);
	
	// unwrap anything marked as unsafe just for markdown
	hookData.edit.contentParsed = placeholder.unwrapLevel(
		hookData.edit.contentParsed, 'markdown');

	hookData = Extensions.runHookChain('mwClientRenderPostMD', hookData);
	content = hookData.edit.contentParsed; toolbar = hookData.toolbar;

	// unsubstitute non-markdown-safe placeholders
	content = placeholder.unwrapPrefixes(content);

	//data.edit.content = new Handlebars.SafeString(content);
	data.edit.formattedContent = content;
	return data;
}
