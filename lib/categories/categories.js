...
function categorySubmitEdit(data) {
	...
	while ((match = re.exec(data.edit.contentParsed)) !== null) {
		name = unformatPageName(match[1].split('|', 2)[0]).toLowerCase();
...
if (Meteor.isServer) {
	...
	Meteor.publish('CategoryChildren', function(categories) {
		categories =
			_.map(categories, function(str) { return str.toLowerCase(); });
	...
	Template.mwCategories.categoriesExist = function() {
		return WikiPages.find({categories: this.name.substr(9).toLowerCase(),
			namespace: 'category'}).fetch().length;
	}

	// Recursive functions, booya! :)
	Template.mwCategories_cats.categories = function() {
		var name = this.name.substr(9).toLowerCase();
